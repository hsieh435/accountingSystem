<template>
  <template v-if="props.stockAccountId">
    <ui-buttonGroup showView :viewText="'檢視證券帳戶'" @dataView="cashCardDataHandling()" />
  </template>
  <template v-if="!props.stockAccountId">
    <ui-buttonGroup showCreate :createText="'新增證券帳戶'" @dataCreate="cashCardDataHandling()" />
  </template>
</template>
...
async function cashCardDataHandling(apiMsg?: string) {
  ...
  Swal.fire({
    ...
    html: `
      <div class="d-flex flex-row items-center rounded-md">
        ...
        <div class="d-flex flex-row justify-start items-center grid grid-cols-6 my-2">
          ...
          <input class="col-span-4 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 px-2 py-1" id="currencyAccountScheme" value="${dataParams.currencyAccountScheme}" ${props.stockAccountId ? "disabled" : ""} />
        </div>
    ...
    preConfirm: () => {
      ...
      if (!props.stockAccountId) {
        dataParams.stockAccountId = getCurrentTimestamp() + "";
      }
